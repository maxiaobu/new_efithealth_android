...
(function(window, undefined){
	...
	TouchSlider.prototype={
		...
		slide:function(index,curid, speed){
			if(curid==null){
			...
			//左右滑动标签页变更，回调页面方法，在mframe.js中也有定义
			slideTabChange(index,curid);
...
function member_clubSlide(index, curid){
	//alert(index+","+curid);
	if (curid == "info") {//资料页
		infoScroll = Number($("body").scrollTop());
	} else if (curid == "course") {//课程页
		courseScroll = Number($("body").scrollTop());
	} else if (curid == "dynamic") {//动态页
		dynamicScroll = Number($("body").scrollTop());
	}
	//alert("infoScroll:"+infoScroll+"courseScroll:"+courseScroll+"dynamicScroll:"+dynamicScroll);
	var tabClassName = $(".ul").className;
	var navH = $(".p_top").height();
	if (index == "0"&&curid!="info") {
		if (isNav&&infoScroll<navH) {
			$("body").scrollTop(infoScroll + $(".p_top").height());
		}else {
			$("body").scrollTop(infoScroll);
		}

	} else if (index == "1"&&curid!="course") {
		if (isNav&&courseScroll<navH) {
			$("body").scrollTop(courseScroll + $(".p_top").height());
		}else {
			$("body").scrollTop(courseScroll);
		}
	} else if (index == "2"&&curid!="dynamic") {
		if (isNav&&dynamicScroll<navH) {
			$("body").scrollTop(dynamicScroll + $(".p_top").height());
		}else {
			$("body").scrollTop(dynamicScroll);
		}
	}
}