...
/*
 * 注释掉的别删，以后可能会用到（注释掉的是下拉刷新headview的相关代码）
 * 
 */
...
//		addHeaderView(mHeaderView);
	 ...
	 * @param enable
	 */
	public void setPullLoadEnable(boolean enable,boolean falg) {
		mEnablePullLoad = enable;
		foot_hd=falg;
		if (!mEnablePullLoad) {
			if (falg) {
				mFooterView.show();
				mFooterView.setState(WaterDropListViewFooter.STATE.stopShow);
