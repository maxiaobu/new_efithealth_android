 ...
 * 发布课程相册显示
 */
public class ChoosePhotoActivity5 extends Activity {
	...
	Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			switch (msg.what) {
			case 0:
				Toast.makeText(ChoosePhotoActivity5.this, "最多选择4张图片", 0)
						.show();
				break;
			case 1:
				if (count == 0) {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		back.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				ChoosePhotoActivity5.this.finish();
			}
		});
		count = Bimp.drrTop.size() + selectTotal;
		if (count == 0) {
			bt.setText("完成");
		} else if (count > 0 && count < 4) {
			bt.setText("完成" + "(" + count + ")");
		...
		public View getView(final int i, View view, ViewGroup viewGroup) {
			...
			viewHolder.imageView.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if ((Bimp.drrTop.size() + selectTotal) < 4) {
						if (!imageModel.getisSelected()) {
						...
						message.what = 1;
					} else if ((Bimp.drrTop.size() + selectTotal) >= 4) {
						if (imageModel.getisSelected()) {
							imageModel.setisSelected(false);// =
							viewHolder.selected.setImageResource(-1);
							selectTotal--;
