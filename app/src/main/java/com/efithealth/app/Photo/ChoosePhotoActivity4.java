 ...
 * 个人信息相册显示
 */
public class ChoosePhotoActivity4 extends Activity {
	...
	Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			switch (msg.what) {
			case 0:
				Toast.makeText(ChoosePhotoActivity4.this, "最多选择12张图片", 0)
						.show();
				break;
			case 1:
				if (count == 0) {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		back.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				ChoosePhotoActivity4.this.finish();
			}
		});
		...
		public View getView(final int i, View view, ViewGroup viewGroup) {
			...
			viewHolder.imageView.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if ((Bimp.drr.size() + selectTotal) < 12) {
						if (!imageModel.getisSelected()) {
						...
						message.what = 1;
					} else if ((Bimp.drr.size() + selectTotal) >= 12) {
						if (imageModel.getisSelected()) {
							imageModel.setisSelected(false);// =
							viewHolder.selected.setImageResource(-1);
							selectTotal--;
